...
private TextView btn_setDate, btn_setTime;
...
public void showDialog() {
    if (!dialog.isShowing()) {
    	btn_setDate.setVisibility(0);
    	btn_setTime.setVisibility(8);
    	if(dateset==false)
    	{
//    		btn_setDate.setVisibility(0);
//        	btn_setTime.setVisibility(8);
        ...
        Log.v("selectedMinute in time set", ""+selectedMinute);
...
//
        ...
        try {
			datePicker.setMinDate(Calendar.getInstance().getTimeInMillis()-3000);
		} catch (Exception e) {
...
//        break;
    ...
    case SET:
        ...
        if(dateset)
        {
        	btn_setDate.setVisibility(8);
        	btn_setTime.setVisibility(0);
        viewSwitcher.showNext();
        dateset=false;
        }
        else if (dialog.isShowing() && isAutoDismiss)
        {
